...
async def get_skills_for_position(position: str) -> list[str]:
    """Parse HH vacancies to extract skills for a given position. Returns list of skills."""
    try:
        async with httpx.AsyncClient(timeout=10) as client:
            resp = await client.get(
                "https://api.hh.ru/vacancies",
                params={"text": position, "per_page": 20, "area": 1},
                headers={"User-Agent": "ResumeBot/1.0"}
            )
            ...
            skills: set[str] = set()
            for v in vacancies:
                # Get full vacancy for key skills
                vid = v.get("id")
                if vid:
                    try:
                        ...
                        if vresp.status_code == 200:
                            vdata = vresp.json()
                            for skill in vdata.get("key_skills", []):
                                name = skill.get("name", "")
                                if name:
                                    skills.add(name)
                    except Exception:
        ...
        logger.error(f"get_skills_for_position error: {e}")
        return []
